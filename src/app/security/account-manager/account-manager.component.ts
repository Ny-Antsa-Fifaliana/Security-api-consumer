...
export class AccountManagerComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  public viewUpdateUser(userName: string, id: number){
    this.router.navigate(['/security','account','update',userName,id]);
  }

  public viewUpdateRole(roleName: string, id: number){
    this.router.navigate(['/security','account','update','role',roleName,id]);
  }

  public viewRoleUser(idUser: number){
    this.router.navigate(['/security','account','role',idUser]);
  }
